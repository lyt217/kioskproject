...
public class JoinMember {// JoinMember클래스 시작
	...
	JoinMember()// JoinMember 생성자 시작
	{
		...
		JLabel age_label = new JLabel("이름");
		...
		// 컴포넌트 사이즈 및 배치설정
		id.setBounds(100, 10, 110, 20);
		pass.setBounds(100, 35, 110, 20);
		tel.setBounds(100, 60, 110, 20);
		age.setBounds(100, 85, 110, 20);
		...
		id_btn.setBounds(220, 5, 90, 25);
		join_btn.setBounds(80, 140, 90, 25);
		close_btn.setBounds(180, 140, 90, 25);
		...
		join.add(panel);
		join.setBounds(width / 3, height / 4, 350, 250);
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (dupl_id == false)// 아이디 중복검사 통과했고 아이디 중복검사 체크를 실행했으면
			{
				...
				if (id.getText().equals("비회원")) {
					...
					String encrypt_Pass = BCrypt.hashpw(String.valueOf(pass.getPassword()),
							BCrypt.gensalt(12));
					JoinMemberProcess.insertMember(id.getText(), encrypt_Pass, tel.getText(), input_age);
